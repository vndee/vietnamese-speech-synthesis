...
from common.datasets.abstract_dataset import AbstractDataset
from common.utils import find_files
...
class CharactorDurationF0EnergyMelDataset(AbstractDataset):
    ...
    def __init__(
        ...
        assert (
            ...
            == len(f0_files)
            == len(energy_files)
        ), f"Number of charactor, mel ({len(mel_files)}), charactor ({len(charactor_files)}), duration ({len(duration_files)}), f0 ({len(f0_files)}) and energy ({len(energy_files)}) files are different"
